...
class WebhooksSetup
{
    ...
    public function triggerDummyEvent($configurations)
    {
        foreach ($configurations as $configuration)
        {
            ...
            sleep(2); // Avoid DB concurrency problems when all webhooks come back
        }
    }
        ...
        foreach ($configurations as &$configuration)
        {
            foreach ($configuration['webhooks'] as $i => $webhook)
            {
                ...
                if (!in_array("product.created", $webhook->enabled_events))
                {
                    ...
                    {
                        $msg = "Failed to update Stripe webhook " . $this->getWebhookUrl() . ": " . $e->getMessage();
    ...
    public function createMissingWebhooks()
    {
        ...
        foreach ($configurations as $secretKey => &$configuration)
        {
            if (empty($configuration['webhooks']))
            {
                ...
                {
                    $msg = "Failed to configure Stripe webhook for store " . $configuration['label'] . ": " . $e->getMessage();
                    $this->errorMessages[] = $msg;
                    Logger::log($msg);
                }
            }
    ...
    public function getAllWebhookConfigurations()
    {
        ...
        foreach ($configurations as $secretKey => &$configuration)
        {
            ...
            {
                $msg = "Failed to retrieve configured webhooks for store " . $configuration['label'] . ": " . $e->getMessage();
                $this->errorMessages[] = $msg;
                Logger::log($msg);
            }
        }
    ...
    {
        \StripeIntegration\Payments\Helper\Logger::log($event);
